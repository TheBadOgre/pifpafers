...
class MoveLayerCommandTest {
    ...
    private fun setup(): Triple<TokenBag, Token, Layer> {
        ...
        t.addLayer(l)
        bag.updateLayerProps(t.id, l.id, l.props.copy(offsetX = 10, offsetY = 20))
    ...
    fun `execute applies both X and Y - undo restores both`() {
        ...
        val cmd = MoveLayerCommand(listOf(
            MoveLayerCommand.Target(t.id, l.id, oldX = 10, oldY = 20, newX = 30, newY = 50)
        ))
        cmd.execute(bag)
        assertEquals(30, t.findLayer(l.id)!!.props.offsetX)
        assertEquals(50, t.findLayer(l.id)!!.props.offsetY)
        cmd.undo(bag)
        assertEquals(10, t.findLayer(l.id)!!.props.offsetX)
        assertEquals(20, t.findLayer(l.id)!!.props.offsetY)
    }
    ...
    fun `merge keeps first old values and last new values`() {
        ...
        val first = MoveLayerCommand(listOf(MoveLayerCommand.Target(tId, lId, 0, 0, 10, 15)))
        val second = MoveLayerCommand(listOf(MoveLayerCommand.Target(tId, lId, 10, 15, 25, 30)))
    ...
    fun `merge rejects different layer sets`() {
        ...
        val first = MoveLayerCommand(listOf(MoveLayerCommand.Target(tId, l1, 0, 0, 5, 5)))
        val second = MoveLayerCommand(listOf(MoveLayerCommand.Target(tId, l2, 0, 0, 5, 5)))
    ...
    fun `merge rejects non-MoveLayerCommand`() {
        val cmd = MoveLayerCommand(listOf(
            MoveLayerCommand.Target(java.util.UUID.randomUUID(), java.util.UUID.randomUUID(), 0, 0, 1, 1)
        ))
